...
import { InternalTransferModalFrame } from './InternalTransferModalFrame'
...
function BroadcastModalShell({ labelId, title, description, onClose, children, testId, contentClassName }: { labelId: string; title: string; description: string; onClose: () => void; children: ReactNode; testId: string; contentClassName?: string }) {
  ...
  return <div ref={overlayRef} className="fixed inset-0 z-[9999] flex items-center justify-center overflow-hidden bg-black/70 px-4 py-5 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === overlayRef.current) onClose() }} role="dialog" aria-modal="true" aria-labelledby={labelId} data-broadcast-modal={testId}><InternalTransferModalFrame ref={panelRef} title={title} description={description} labelId={labelId} onClose={onClose} className="overflow-hidden" contentClassName={contentClassName} descriptionClassName="text-sm" descriptionWidthClassName="max-w-[560px]" style={{ maxHeight: 'calc(100dvh - 40px)' }}>{children}</InternalTransferModalFrame></div>
}
...
export function BroadcasterTermsModal({ onCancel, onContinue }: { onCancel: () => void; onContinue: () => void }) {
  ...
  return <BroadcastModalShell labelId="broadcaster-terms-title" title="Broadcaster Terms & Conditions" description="You must re-agree to the streaming terms every time you go live." onClose={onCancel} testId="terms" contentClassName="px-5 pb-5 pt-5 sm:px-[42px] sm:pb-8 sm:pt-8"><h3 className="text-center text-2xl font-normal text-white">Streaming Terms</h3><div className="mx-auto mt-4 h-[170px] w-[519px] max-w-full overflow-y-auto rounded-[18.563px] border border-[#064B34] p-5 text-xs leading-[16px] text-gfx-neutral-400 sm:mt-6 sm:h-[270px]" data-broadcast-terms-scroll role="region" aria-label="Broadcaster terms, scrollable" tabIndex={0}><p>{TERMS_COPY}</p><h4 className="mt-5 font-medium text-white">1. Definitions</h4><p className="mt-2">&quot;Broadcaster&quot; means the verified account holder who transmits a Stream</p></div><label htmlFor={checkboxId} className="mt-4 flex cursor-pointer items-start gap-3 text-xs leading-[16px] text-white sm:mt-5"><input id={checkboxId} type="checkbox" checked={accepted} onChange={(event) => setAccepted(event.target.checked)} className="peer sr-only"/><span className="grid size-5 shrink-0 place-items-center rounded-full bg-[#00B38C]/25 text-[#00B38C] peer-focus-visible:ring-2 peer-focus-visible:ring-[#00B38C]"><CheckIcon/></span><span>I have read and agree to the Broadcaster Terms &amp; Conditions, and I<br className="hidden sm:block"/> consent to turning on my camera and microphone and to my stream<br className="hidden sm:block"/> being recorded.</span></label><div className="mt-4 flex flex-col justify-center gap-2 sm:mt-6 sm:flex-row"><SparkleButton onClick={onCancel} className="!h-[46px] !w-[230px] max-w-full px-6">Cancel</SparkleButton><GlowButton label="Continue" width={230} height={44} disabled={!accepted} onClick={onContinue}/></div></BroadcastModalShell>
}
...
export function BroadcastPermissionsModal({ active, onEnable, onCancel, onStart }: { active: BroadcastPermission[]; onEnable: (permission: BroadcastPermission) => void; onCancel: () => void; onStart: () => void }) {
  ...
  return <BroadcastModalShell labelId="broadcast-permissions-title" title="Grant broadcast permissions" description="Camera, microphone, and screen share must all be approved before your stream can begin." onClose={onCancel} testId="permissions" contentClassName="px-4 pb-5 pt-5 sm:px-[42px] sm:pb-8 sm:pt-8"><h3 className="text-center text-2xl font-normal text-white">Required permissions</h3><div className="mx-auto mt-4 w-[520px] max-w-full space-y-3 sm:mt-6" data-broadcast-permission-rows>{PERMISSION_ROWS.map((row) => { const enabled = active.includes(row.id); return <div key={row.id} className="flex h-[100px] items-center gap-2 rounded-[18.563px] border border-[#064B34] px-3 sm:h-[91px] sm:gap-4 sm:px-5"><span className="grid size-10 shrink-0 place-items-center rounded-[10px] bg-[#064B34]/55 text-[#00B38C] sm:size-[46px]"><PermissionIcon permission={row.id}/></span><div className="min-w-0 flex-1"><h4 className="text-sm text-white">{row.title}</h4><p className="mt-0.5 text-xs leading-[15px] text-gfx-neutral-400">{row.copy}</p></div>{enabled ? <span className="flex h-10 w-[74px] shrink-0 items-center justify-center gap-1 rounded-full border border-[#00B38C] text-xs text-[#00B38C] sm:w-[87px]" data-permission-active><span className="grid size-4 place-items-center rounded-full bg-[#00B38C]/25"><CheckIcon className="size-2.5"/></span>Active</span> : <GlowButton label="Enable" width={103} height={44} fontSize={14} className="!w-[84px] sm:!w-[103px]" onClick={() => onEnable(row.id)}/>}</div>})}</div><p className="mt-4 text-xs leading-[16px] text-gfx-neutral-400 sm:mt-5">Denied a permission by mistake? Enable it in your browser’s site<br className="hidden sm:block"/> settings, then tap the button again</p><div className="mt-4 flex flex-col justify-center gap-2 sm:mt-6 sm:flex-row"><SparkleButton onClick={onCancel} className="!h-[46px] !w-[230px] max-w-full px-6">Cancel</SparkleButton><GlowButton label="Start streaming" width={230} height={44} disabled={!allActive} preserveDisabledAppearance onClick={onStart} icon={<BroadcastGlyph className="size-[15px]"/>}/></div></BroadcastModalShell>
}
