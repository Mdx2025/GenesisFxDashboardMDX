    ...
    <div ref={ref} className={`relative ${open ? 'z-50' : 'z-0'}`}>
      ...
      <button
        ...
        className={`w-full flex items-center cursor-pointer focus:outline-none h-[50px] bg-gfx-green-100 rounded-2xl ${
          open ? 'border border-gfx-focus-ring' : 'border border-gfx-neutral-350'
        }`}
      ...
      <ul
        ref={dropdownRef}
        role="listbox"
        className="absolute left-0 right-0 top-full mt-2 rounded-2xl overflow-hidden z-50 border border-gfx-card-border bg-gfx-surface-panel [backdrop-filter:url(#blur-24)]"
      >
        {options.map((option) => {
