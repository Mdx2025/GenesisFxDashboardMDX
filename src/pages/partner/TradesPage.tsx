...
type TabOption = 'Open Positions' | 'Closed Trades'
...
function TradeTabs({ active, onChange }: { active: TabOption; onChange: (tab: TabOption) => void }) {
  const tabs: TabOption[] = ['Open Positions', 'Closed Trades']
  return (
    <div className="bg-gfx-green-800 rounded-[60px] inline-flex items-center gap-[31px] pl-[5px] pr-4 py-[3px]">
      {tabs.map(tab => {
        ...
        return (
          <button
            ...
            className={`relative text-xs font-acid whitespace-nowrap cursor-pointer transition-all leading-[18.8px] ${
              isActive
                ? 'bg-gfx-green-200 border-[1.35px] border-gfx-green-500 text-white rounded-[60px] px-4 py-1.5 overflow-hidden w-[140px] text-center'
          ...
          >
            ...
            {isActive && (
              <svg className="absolute left-1/2 -translate-x-1/2 bottom-[-12px]" width="54" height="33" viewBox="0 0 54 33" fill="none" style={{ mixBlendMode: 'plus-lighter' }}>
                <g filter="url(#trade_tab_glow)">
                  <ellipse cx="27" cy="35" rx="14.5" ry="8" fill="#CFF2E6" />
                </g>
                <defs>
                  <filter id="trade_tab_glow" x="-27.5" y="-13" width="109" height="96" filterUnits="userSpaceOnUse" colorInterpolationFilters="sRGB">
...
export default function TradesPage() {
  ...
  const [activeTab, setActiveTab] = useState<TabOption>('Open Positions')
