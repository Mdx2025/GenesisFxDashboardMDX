...
async function modalFrameContract(name) {
  return page.locator(`[data-broadcast-modal="${name}"]`).evaluate((overlay) => {
    ...
    return {
      ...
      close: { rightInset: frameBox.right - closeBox.right, topInset: closeBox.top - frameBox.top },
    }
  })
}
...
if (!termsFrameContract.usesInternalTransferFrame || Math.abs(termsFrameContract.frame.width - 793) > 1 || termsFrameContract.frame.radius !== '30px' || Math.abs(termsFrameContract.inner.width - 701) > 2 || termsFrameContract.inner.radius !== '24px' || Math.abs(termsFrameContract.close.rightInset - 28) > 1 || Math.abs(termsFrameContract.close.topInset - 28) > 1) failures.push(`terms internal-transfer frame mismatch: ${JSON.stringify(termsFrameContract)}`)
...
if (!permissionsFrameContract.usesInternalTransferFrame || Math.abs(permissionsFrameContract.frame.width - 793) > 1 || permissionsFrameContract.frame.radius !== '30px' || Math.abs(permissionsFrameContract.inner.width - 701) > 2 || permissionsFrameContract.inner.radius !== '24px' || Math.abs(permissionsFrameContract.close.rightInset - 28) > 1 || Math.abs(permissionsFrameContract.close.topInset - 28) > 1) failures.push(`permissions internal-transfer frame mismatch: ${JSON.stringify(permissionsFrameContract)}`)
