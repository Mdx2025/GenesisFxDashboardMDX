...
- Production validation: Dokploy `CtQkxTzuYjJGplKsNyPR6` finished `done` at `2026-08-22T00:25:31.694Z`; `/streaming` returned `200`; remote QA confirmed rail padding `0px`, rail `44px`, active pill `46px` with `1px` protrusion on both vertical edges, keyboard activation, favorites empty state, three-card restoration, four desktop grid tracks, light/dark focus paint, zero overflow, runtime errors, or failed responses. Contrast ratios: active text `20.42:1`, inactive text `5.15:1` dark / `7.54:1` light, focus `8.27:1` dark / `9.46:1` light. Automated axe/Lighthouse was not available in the repo or host; native-button semantics, `aria-pressed`, live-region status, focus, keyboard, reduced-motion, and responsive checks passed in the project browser harness.
