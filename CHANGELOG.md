...
- Gave the `/news/ai-analysis` toolbar real effect on the analysis it sits above. Every card on the screen read the module-level `EUROUSD` fixture directly, so the seven instrument pills only moved a highlight and the `Analyze` button had nowhere to send the symbol: whatever you selected or typed, the page still showed EURUSD at 1.1443. `src/data/aiAnalysis.ts` now exposes `buildAiAnalysis(symbol)`, which returns the handwritten EUROUSD copy verbatim and derives the whole dataset — quote, sentiment gauge, hourly bars, indicator tiles, recent range, technical signals, key levels, trade idea and both narratives — for anything else. The generated numbers come from a mulberry32 PRNG seeded with an FNV-1a hash of the ticker, so a symbol always renders identically across re-renders, reloads and back-navigation instead of reshuffling while you type; a ticker is classified as FX, Crypto, Index, Metals, Commodity or Stocks before its price is drawn, so `BTCUSD` quotes at a 68k handle with two decimals rather than at a 1.14 FX handle, and `USDJPY` keeps three. The derived values are made to agree with each other rather than drawn independently: the sentiment gauge is computed from the signal tally, the range percentage from the low/high it prints, and the levels, entry, stop and targets from the same price and step, so the trade idea can never contradict the table beside it. `AiAnalysisView` takes the dataset through props into every card, the pills select by symbol instead of by index, and the toolbar is now a `<form>` so Enter analyses as well as the button; a searched symbol that is not one of the seven defaults joins the pill row as the selected chip. `eur/usd` and `btc-usd` normalise to the bare uppercase ticker. Only EUROUSD ships flag artwork, so a generated instrument gets a monogram tile at the same 87×80.5 geometry instead of borrowing the euro flag, and the change badge now flips to the bearish tone and arrow when the generated move is negative. The quote sparkline stays the fixed Figma silhouette — it is decorative art with no data input.

- Made the `/news/ai-analysis` toolbar field drive its `Analyze` button. The field is where you name the instrument to analyse, not a catalogue search, so its placeholder and accessible name now read `Symbol` instead of `Search markets`. `AiAnalysisView` holds the value in state and `Analyze` is `disabled` while it is empty or whitespace, so the button can no longer be pressed with nothing to analyse. `SparkleButton` gained the disabled treatment it was missing — its hover CSS was already written as `:not(:disabled)`, but the rest state had no `cursor-not-allowed` or dimming, so a disabled sparkle button looked identical to an enabled one; it now uses the same `0.4` opacity as `GlowButton`.

- Routed all three `View Commission Rates` buttons to `/partner/comissions`. The same referral-link card is repeated on `/partner`, `/partner/links` and `/partner/marketing`, and in every copy the button under the referral URL was rendered without an `onClick`, so the commission table was reachable only from the sidebar. Each host component now takes `useNavigate` and sends the click to the registered `/partner/comissions` route. The label on the marketing copy still reads `View Comission Rates` and the route itself is spelled `comissions`; both typos are pre-existing and left as they are so this change stays a routing fix.

- Pointed the `/partner` Partner Wallet card's `Request Payout` button at `/partner/payouts`. The card shows the partner balance and its single button was the natural way to start a payout, but it was rendered without an `onClick`, so the payout flow was only reachable from the sidebar even though `/partner/payouts` was already a registered route and its page leads with a `Request Payout` form. `PartnerWalletCard` now takes `useNavigate` the same way `HomeState` and `MyStreaming` do and routes the click there.

- Wired the `/partner` Partner Program banner's two calls to action. `Marketing Tools` and `View statistics` both rendered without an `onClick`, so the banner's only two entry points were inert even though `/partner/marketing` and `/partner/statistics` were already registered routes. `Marketing Tools` now lands on the marketing page's `Landing Pages` tab and `View statistics` goes to `/partner/statistics`. To make the tab reachable from outside the page, `MarketingPage` reads an optional `?tab=` slug (`marketing-library`, `landing-pages`, `referral-links`) as the initial tab, following the `useSearchParams` pattern already used by `WithdrawProcessingPage` and the Academy single pages; the slugs are derived from `TABS` rather than written twice, and an unknown or absent value falls back to the first tab. Clicking tabs in the page still uses local state, so this adds a deep link without turning every tab click into a history entry.
...
- Added the `/streaming/watch` viewer page and pointed `Watch now` at it. The button on `/streaming` was inert — it rendered the featured stream's call to action with no handler, so the only entry point into a broadcast went nowhere. The new page is Figma frame `1000002408` (`4037:109037`) brought over against measured geometry rather than eyeballed: the chart toolbar keeps Figma's irregular label centres (125/166/228.5/291/340/376.5/416) on an absolutely positioned 660px track that scrolls instead of reflowing, because uniform flex spacing drifts several pixels off by `1D`. The chart raster is the existing `streaming-chart.png` placed oversized and offset (`106.83%` wide at `-3.506%`/`-8.185%`) so the source image's own price axis stays cropped out, matching the frame; `object-cover` had left it visible. The broadcaster picture-in-picture and the chart's date axis are written in `em` against a root `font-size: calc(100cqw / 1548)`, so one `em` is one design pixel scaled by the panel's own width: both keep their Figma geometry exactly at the 1548px design width and shrink with the panel instead of swallowing it on a phone, where the fixed 385px pip covered the whole chart and the fixed 74px axis claimed 43% of its height. Sixteen icons were traced from the frame's own vector data — play-circle, cursor, gift, heart, viewers eye, send plane, the pip's play/mute/frame/dots/expand/collapse glyphs, the three chart-type glyphs and the help circle — and the `Follow` icon reuses the existing `FollowPersonIcon` byte for byte rather than adding a duplicate. `StreamingLiveBadge` gained a `success` tone with an optional dot for the green `LIVE` chip, and `GlowEllipse` gained a `chat` variant; its opacity was tuned to `90` after measuring the frame's own glow falloff, since `45` rendered it roughly 2.4× too faint against the panel.
